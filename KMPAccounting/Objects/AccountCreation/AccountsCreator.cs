        ...
        public static void CreateAccounts(this Ledger ledger, DateTime dateTime, Type t, object? o = null)
        {
            var allItems = t.GetProperties().Where(x =>
                ...
                .Concat(t.GetFields().Where(x =>
                {
                    ...
                    (x?.GetValue(o)?.ToString(), x?.GetCustomAttribute<OppositeToParentAttribute>() != null)))
                .OrderBy(pair => pair.Item1);

            foreach (var (accountPath, oppositeToParent) in allItems)
            {
                ledger.EnsureCreateAccount(dateTime, accountPath!,
                    AccountHelper.GetChooseSideFunc(oppositeToParent
                        ? AccountHelper.SideOptionEnum.OnlyLeafOppositeToParent
                        : AccountHelper.SideOptionEnum.SameAsParent));
            }
        }
    }
}